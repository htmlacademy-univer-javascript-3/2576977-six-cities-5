...
export const ReviewForm: FC = () => {
  ...
  const handleRatingChange = (event: React.ChangeEvent<HTMLInputElement>) => setRating(parseInt(event.target.value, 10));
  ...
  return (
    <form className="reviews__form form" action="#" method="post">
      ...
      <div className="reviews__button-wrapper">
        ...
        <button className="reviews__submit form__submit button" type="submit" disabled={reviewText.length < MINIMUM_REVIEW_LENGTH || rating === 0}>Submit</button>
      </div>
    </form>
  );
};
